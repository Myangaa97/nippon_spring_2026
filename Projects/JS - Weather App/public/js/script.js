...
function updateWeatherDOM() {
    ...
    let currentImage = weatherCode;
    if (isDay === 0) {
        currentImage = `${weatherCode}_night`;
    }

    let tempUnit = '°C';
    let speedUnit = 'KM/H';
    if (currentUnit === 'F') {
        maxTemp = parseFloat(((maxTemp * 9) / 5 + 32).toFixed(1));
        minTemp = parseFloat(((minTemp * 9) / 5 + 32).toFixed(1));
        feelsLike = parseFloat(((feelsLike * 9) / 5 + 32).toFixed(1));
    ...
    document.querySelector('.city-temp').textContent = `${(maxTemp)}°/${(minTemp)}°`;
    document.querySelector('.city-w-img').innerHTML = `<img src="./images/${currentImage}.png" alt="${weatherCode}">`;
    ...
    weatherData.hourly.time.forEach((time, index) => {
        ...
        const isHourlyNight = hour < sunriseHour || hour >= sunsetHour;
        let hourImage = hourWeathercode;
        if(isHourlyNight) {
            hourImage = `${hourWeathercode}_night`;
        }
        const card = document.createElement('div');
        card.classList.add('forecast-card');
        card.innerHTML = `
            <p>${formatTime(time)}</p>
            <img src="./images/${hourImage}.png" alt="${hourWeathercode}"></img>
            <h4>${(temp)}°</h4>
        `;
    ...
    weatherData.daily.time.forEach((date, index) => {
        ...
        if (currentUnit === 'F') {
            maxTempWeek = parseFloat(((maxTempWeek * 9) / 5 + 32).toFixed(1));
            minTempWeek = parseFloat(((minTempWeek * 9) / 5 + 32).toFixed(1));
        } else {
            maxTempWeek = Math.round(maxTempWeek);
            minTempWeek = Math.round(minTempWeek)
        }
        ...
        card.innerHTML = `
            ...
            <p class="week-meter">${(maxTempWeek)}°/${(minTempWeek)}°</p>
        `;
...
async function searchCities(city) {
    try {
        ...
        if (!data.results || !data.results.length === 0) {
            ...
            noResultItem.classList.add('city-suggestion');
            noResultItem.style.cursor = 'default';
            noResultItem.style.color = 'rgba(33, 33, 33, 0.87)'
            noResultItem.innerHTML = `<span style="font-weight: 400;">City not found</span>`;
